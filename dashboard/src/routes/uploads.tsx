...
function UploadsPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Page Header */}
      <div>
        <h2 className="text-xl font-bold tracking-tight text-white m-0">Transfer Logs</h2>
        <p className="text-xs text-gray-500 m-0 mt-1">Audit trail and status logs of all file uploads piped through the engine.</p>
      </div>
      ...
      <div className="glass-card flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
          ...
          <div className="flex flex-col gap-1 w-full md:w-48">
            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Filter Status</label>
            <select
              value={statusFilter}
          ...
          <div className="flex flex-col gap-1 w-full md:w-56">
            <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500">Filter Remote</label>
            <select
              value={accountFilter}
      ...
      <div className="glass-card">
              ...
              <tbody className="divide-y divide-solid divide-white/5 text-xs">
                {transfersData.data.map((tx) => (
                  <tr key={tx.id} className="hover:bg-white/2 transition-colors">
                    <td className="py-3.5 pl-2 max-w-[200px] truncate font-medium text-gray-200" title={tx.fileName}>
                      {tx.fileName}
                    </td>
                    <td className="py-3.5 max-w-[250px] truncate text-gray-400 font-mono text-[11px]" title={tx.filePath}>
                      {tx.filePath}
                    </td>
                    <td className="py-3.5 text-gray-400 font-mono text-[11px]">
                      {accountsList?.find((a) => a.id === tx.accountId)?.name || tx.accountId.slice(0, 8)}
                    </td>
                    <td className="py-3.5 font-mono text-gray-400">{formatBytes(tx.fileSize)}</td>
                    <td className="py-3.5">
                      <span
                      ...
                      {tx.error && (
                        <p className="text-[10px] text-red-400/80 m-0 mt-1 max-w-[200px] truncate" title={tx.error}>
                          {tx.error}
                        </p>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 pr-2 text-right text-[11px] text-gray-500 font-mono">
        ...
        {transfersData && transfersData.total > limit && (
          <div className="flex items-center justify-between border-t border-solid border-white/5 pt-4 mt-6">
